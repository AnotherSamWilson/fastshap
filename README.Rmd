...
1) Fast. See benchmarks for comparisons. Speed increase is only limited by your RAM and the model calculation time.  
2) Native handling of both numpy arrays and pandas dataframes including principled treatment of categories.  
2) Easy built in stratification of background set.  
3) Capable of plotting categorical variables in dependence plots.  
4) Capable of determining categorical variable interactions in shap values.  
...
knitr::include_graphics("C:/Users/swilson/Projects/fastshap/benchmarks/iris_benchmark_time.png")
plot_df %>%
  kbl() %>%
  kable_minimal() %>% 
...
The California Housing dataset is a table of 20640 rows and 9 columns (8 features, one target). This benchmark measured the time it took to calculate shap values on the first 2000 rows for different sizes of background datasets.  
```{r,eval=TRUE,echo=FALSE,out.width="600px"}
plot_df <- data.table::fread("benchmarks/cali_benchmark_time_df.csv")[,c(2,3,4,5)]
plot_df[,c(2,3,4)] = round(plot_df[,c(2,3,4)], 2)
knitr::include_graphics("C:/Users/swilson/Projects/fastshap/benchmarks/cali_benchmark_time.png")
plot_df %>%
  kbl() %>%
  kable_minimal() %>% 
  # kable_styling(latex_options = c("striped", "hold_position"), full_width = FALSE) %>% 
...
```{r,eval=TRUE,echo=FALSE,out.width="600px"}
knitr::include_graphics("C:/Users/swilson/Projects/fastshap/benchmarks/batch_size_times.png")
``` 
...
knitr::include_graphics("C:/Users/swilson/Projects/fastshap/graphics/depgraph.png")
```  
...
```{r,eval=TRUE,echo=FALSE,out.width="800px"}
knitr::include_graphics("C:/Users/swilson/Projects/fastshap/graphics/depgraph_cat.png")
```  
...
```
```{r,eval=TRUE,echo=FALSE,out.width="800px"}
knitr::include_graphics("C:/Users/swilson/Projects/fastshap/graphics/multiclass_depgraph.png")
```  
...
```
```{r,eval=TRUE,echo=FALSE,out.width="800px"}
knitr::include_graphics("C:/Users/swilson/Projects/fastshap/graphics/mc_so_depgraph.png")
```  
